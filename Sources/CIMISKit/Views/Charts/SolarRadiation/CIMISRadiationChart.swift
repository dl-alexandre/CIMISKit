...
public struct CIMISRadiationChart: View {
 ...
 public var body: some View {
        GeometryReader { geo in
            ScrollView(.horizontal) {
                VStack {
                    Label("Solar Radiation (Langly/Day)", systemImage: "sun.max")
                        .bold()
                        .padding()
                    ...
                    Chart(chartValues, id: \.id) {
                        LineMark(
                            x: .value("Date", $0.date),
                            y: .value("Max", Double($0.dailySolarRadiationAvg.value ?? "0")!)
                        ).foregroundStyle(.red)
                    }
...
//#Preview {
//    CIMISRadiationChart(chartValues: Providers.dailyData.cimisData.providers[0].records, width: 500)
//}
